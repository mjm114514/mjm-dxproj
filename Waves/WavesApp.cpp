...
void WavesApp::Draw(const GameTimer& gt)
{
	...
	DrawRenderItems(mCommandList.Get(), mRitemLayer[(int)RenderLayer::AlphaTestedTreeSprite]);
...
void WavesApp::UpdateMaterialCBs(const GameTimer& gt)
{
...
std::array<const CD3DX12_STATIC_SAMPLER_DESC, 6> WavesApp::GetStaticSamplers(){
...
void WavesApp::BuildPostProcessRootSignature(){
...
void WavesApp::BuildDescriptorHeaps(){
    // Create SRV heap

    if(mTextures.size() > 0){
        D3D12_DESCRIPTOR_HEAP_DESC srvHeapDesc = {};
        srvHeapDesc.NumDescriptors = (UINT)mTextures.size() + mWaves->DescriptorCount() + mSobelFilter->DescriptorCount() + 1;
    ...
    auto fenceTex = mTextures["crate"]->Resource;
	auto treeSpriteTex = mTextures["treeSprite"]->Resource;
    ...
    handle.Offset(mWaves->DescriptorCount(), mCbvSrvDescriptorSize);
    gHandle.Offset(mWaves->DescriptorCount(), mCbvSrvDescriptorSize);
    ...
    handle.Offset(mSobelFilter->DescriptorCount(), mCbvSrvDescriptorSize);
    gHandle.Offset(mSobelFilter->DescriptorCount(), mCbvSrvDescriptorSize);
...
void WavesApp::BuildBoxGeometry(){
    GeometryGenerator geoGen;
...
void WavesApp::BuildPSOs()
{
	...
	treeSpritesPsoDesc.VS = {
		reinterpret_cast<BYTE*>(mShaders["treeSpriteVS"]->GetBufferPointer()),
		mShaders["treeSpriteVS"]->GetBufferSize()
	};
	treeSpritesPsoDesc.GS = {
		reinterpret_cast<BYTE*>(mShaders["treeSpriteGS"]->GetBufferPointer()),
		mShaders["treeSpriteGS"]->GetBufferSize()
	};
	treeSpritesPsoDesc.PS = {
    ...
    ThrowIfFailed(md3dDevice->CreateGraphicsPipelineState(
        &compositePsoDesc,
        IID_PPV_ARGS(&mPSOs["composite"])
    ));

    // PSO for waves Disturb
...
void WavesApp::LoadTextures(){
    ...
    ThrowIfFailed(CreateDDSTextureFromFile12(
        md3dDevice.Get(),
        mCommandList.Get(),
        crate->Filename.c_str(),
        crate->Resource,
        crate->UploadHeap
    ));

	auto treeSprite = std::make_unique<Texture>();
	treeSprite->Name = "treeSprite";
	treeSprite->Filename = L"..\\Textures\\treeArray2.dds";
	ThrowIfFailed(CreateDDSTextureFromFile12(
		md3dDevice.Get(),
		mCommandList.Get(),
		treeSprite->Filename.c_str(),
		treeSprite->Resource,
		treeSprite->UploadHeap
	));

    mTextures["grass"] = std::move(grass);
    mTextures["water"] = std::move(water);
    mTextures["crate"] = std::move(crate);
	mTextures["treeSprite"] = std::move(treeSprite);
}
...
void WavesApp::BuildRenderItems()
{
    ...
    XMStoreFloat4x4(&gridRitem->TexTransform, XMMatrixScaling(5.0, 5.0, 1.0));
	...
	treeSpriteRitem->Mat = mMaterials["treeSprite"].get();
	...
	mRitemLayer[(int)RenderLayer::AlphaTestedTreeSprite].push_back(treeSpriteRitem.get());
...
void WavesApp::DrawFullscreenQuad(ID3D12GraphicsCommandList* cmdList){
