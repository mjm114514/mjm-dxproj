...
VertexOut VS(VertexIn vin)
{
    ...
    vin.PosL.y += gDisplacementMap.SampleLevel(gsamPointClamp, vin.TexC, 1.0f).r;

    // Estimate normal using finite difference.
    float du = gDisplacementMapTexelSize.x;
    float dv = gDisplacementMapTexelSize.y;
    float l = gDisplacementMap.SampleLevel(gsamPointClamp, vin.TexC - float2(du, 0), 0.0f).r;
    float r = gDisplacementMap.SampleLevel(gsamPointClamp, vin.TexC + float2(du, 0), 0.0f).r;
    float t = gDisplacementMap.SampleLevel(gsamPointClamp, vin.TexC + float2(0, dv), 0.0f).r;
    float b = gDisplacementMap.SampleLevel(gsamPointClamp, vin.TexC - float2(0, dv), 0.0f).r;

    vin.NormalL = normalize(float3(-l + r, 2.0f * gGridSpatialStep, b - t));
#endif
